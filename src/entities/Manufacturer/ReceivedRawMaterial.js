...
export default function ReceivedRawMaterial(props) {
    ...
    let display = (
      <div>
        ...
        <p>
          ...
          <Link
            to={{
              pathname: `/manufacturer/view-transaction/${data[6]}`,
  ...
  async function saveRawMaterialDetails() {
    ...
    let rawMaterialInfoData = await rawMaterial.methods
      .getSuppliedRawMaterials()
      .call({ from: account });

    let transaction = new web3.eth.Contract(
      Transactions.abi,
      rawMaterialInfoData[6]
    );
    ...
    axios
      .post("http://localhost:8000/api/raw-material/save-details", {
  ...
  if (loading) {
    ...
    return (
      <div>
        ...
        >
          Save Raw Material Info to Database
        </Button>
      </div>
    );
  }
